export class SwarmOrchestrator {
    constructor() {
        ...
        this.lastRebalance = new Map();
        this.rebalanceInterval = 1000;
    }
    ...
    createAgents(count, topology) {
        const roles = ["coordinator", "executor", "reviewer", "optimizer"];
        const agents = [];
        for (let i = 0; i < count; i++) {
            const role = topology === "hierarchical" && i === 0 ? "coordinator" : (roles[i % roles.length]);
            agents.push({
                id: `agent-${Date.now()}-${i}`,
                name: `Agent-${i}`,
                role,
    ...
    rebalanceIfNeeded(swarmId, swarm) {
        const now = Date.now();
    ...
    performWorkStealing(swarm) {
        if (swarm.agents.length < 2)
            return;
        ...
        const predictions = agents.map((agent) => ({
            ...
            predictedTime: agent.currentLoad *
        ...
        const slowest = predictions.reduce((prev, current) => prev.predictedTime > current.predictedTime ? prev : current);
        const fastest = predictions.reduce((prev, current) => prev.predictedTime < current.predictedTime ? prev : current);
        if (slowest.queue &&
            fastest.queue &&
    ...
    releaseTask(swarmId, agentId, success, taskId) {
    ...
    calculateHealthScore(swarm) {
        const total = swarm.metrics.totalTasks;
        if (total === 0)
            return 1.0;
        const successRate = swarm.metrics.completedTasks / total;
        const avgLoad = swarm.agents.reduce((sum, a) => sum + a.currentLoad, 0) / swarm.agents.length;
